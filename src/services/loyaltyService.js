...
async function awardOncePerBooking(tx, userId, activityId) {
  const activity = await tx.activity_type.findUnique({ where: { activity_id: activityId } });
  await tx.xp_log.create({
    data: {
      user_id: userId,
...
export default { checkActivities, checkDepositActivity };
